...
__all__ = ['IntegerizePACTConvPass',
           ...
           'PACT_symbolic_trace',
           'PACTIntegerHardGLU',
           'IntegerizeHardGLUPass',
           'IntegerizeMaskSoftmaxPass'
           ]
...
def integerize_hardglu_fun(gm : fx.GraphModule, match : Match, D=2**14, export_node = False):
    modules = gm_modules(gm)
    matched_nodes = [m for k, m in match.nodes_map.items() if k.op == 'call_module']
    lin_node = matched_nodes[0]
    matched_modules = [modules[m.target] for k, m in match.nodes_map.items() if k.op == 'call_module'][::-1]
...
class IntegerizeMaskSoftmaxPass(ModularizePass):
    @staticmethod
    ...
    def __init__(self, D=2**12, export_masksoftmax_node = False, **kwargs):
        self.kwargs = kwargs
        target = [PACTMaskSoftmax()]
        super().__init__(op='call_module', target=tuple(target), replacement_fn = partial(self.integerize_masksoftmax_fun, integer_node=export_masksoftmax_node), name="INTEGER_MASKSOFTMAX_REPLACEMENT_PASS")
